...
int interval=50; // the time we need to wait for fade (fade slower at 30, Fade fast at 10).
int MusicInterval=2; //15; // the time we need to wait Music (30 seems good) 15 is the limit.
...
void setup() {
  ...
  pinMode(buttonPin, INPUT);
   digitalWrite(buttonPin, LOW);  // button pin is HIGH, so it drops to 0 if pressed
...
//Serial.println(motionValue);
  ...
  //LEDS.clear();
    if (buttonVal == HIGH && oldButtonVal == LOW) {// button has just been pressed
...
void fadeall() { for(int i = 0; i < NUM_LEDS; i++) { leds[i].nscale8(210); } } //Fade to black by 25%
...
for (int i=0;i<7;i++){
    ...
    spectrumValue[i]=map(spectrumValue[i], filter,1023,0,254); //test point
 ...
 if ((unsigned long)(currentMillis - previousMusicMillis) >= MusicInterval) {
  //  for (int i=0;i<4;i++)
  //{
  int bass = map((spectrumValue[0]+spectrumValue[1])/2, filter, 254, 0, length);
  ...
  int high = map(spectrumValue[6], filter, 254, 0, length);
    ...
    for(int led = 0; led < low; led++) { 
        leds[-led-1+(length*2)] = ColorFromPalette(OceanColors_p, low+random(0,lHue), low+random(BRIGHTNESS_Low,BRIGHTNESS_High), LINEARBLEND);
    }
